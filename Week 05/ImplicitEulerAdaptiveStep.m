function [T,X,info] = ImplicitEulerAdaptiveStep(funJac,t0,tf,x0,h0,abstol,reltol,varargin)
    ...
    while t < tf
        ...
        while ~AcceptStep
            %Take step of size h
            x1 = x+ h*f;
            [~,nfunNewt] = NewtonsMethodODE(func, t, x, h, x1, epstol, maxit, varargin{:});
            nfun = nfun+nfunNewt;
            ...
            tm = t+hm;
            xm = x+hm*f;
            [fm,nfunNewt] = NewtonsMethodODE(func, t, x, hm, xm, epstol, maxit, varargin{:});
            fm2 = feval(funJac, tm,fm,varargin{:});
            nfun = nfun + nfunNewt + 1;
            xinit= fm+hm*fm2;
            [x1hat,nfunNewt] = NewtonsMethodODE(func, tm, fm,hm,xinit,epstol,maxit,varargin{:});
            nfun = nfun + nfunNewt;
