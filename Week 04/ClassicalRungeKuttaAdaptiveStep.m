function [T,X,info] = ClassicalRungeKuttaAdaptiveStep(fun,t0,tf,x0,h0,abstol,reltol,varargin)

    %Error controller parameters
    epstol = 0.8; % target
    kpow = 0.2;
    facmin = 0.1; % maximum decrease factor
    facmax = 5.0; % maximum increase factor
    
    %Initial conditions
    t = t0;
    h = h0;
    x = x0;
    
    %Output
    T=t;
    X=x';
    
    % Information output
    nfun = 0;
    hvec = [];
    rvec = [];
    err = [];
    rr = [];
    hh =[];

    %% Algorithm
    while t < tf
        if (t+h>tf)
            h = tf-t;
        end 
        f = feval(fun,t,x,varargin{:});
        nfun = nfun+1;

        AcceptStep = false;
        while ~AcceptStep
            %Take step of size h
            [~, x1] = ClassicalRungeKuttaStep(fun,t,x,f,h,varargin{:});

            %Take step size h/2
            hm = 0.5*h;
            [tm, xm] = ClassicalRungeKuttaStep(fun,t,x,f,hm,varargin{:});
            fm = feval(fun,tm,xm,varargin{:});
            [~,x1hat] = ClassicalRungeKuttaStep(fun,tm,xm,fm,hm,varargin{:});
            nfun = nfun+1;

            %Error estimation
            e = x1hat-x1;
            r = max(abs(e)./max(abstol,abs(x1hat).*reltol));

            AcceptStep = (r <= 1.0);
            if AcceptStep
                t = t+h;
                x = x1hat;

                T = [T;t];
                X = [X;x'];
                
                err = [err;e.'];
                rr = [rr;r.'];
                hh = [hh;h.'];
            end
            %asymptotic step size controller
            h = max(facmin,min((epstol/r)^kpow,facmax))*h;
            hvec(end+1) = h;
            rvec(end+1) = r;
        end
        
    info.nfun = nfun;
    info.naccept = length(T);
    info.nreject = length(hvec) - length(T) + 1;
    info.nstep = length(hvec) + length(rvec);
    info.hvec = hvec;
    info.rvec = rvec;
    info.err = err;
    info.rr = rr;
    info.hh = hh;
end